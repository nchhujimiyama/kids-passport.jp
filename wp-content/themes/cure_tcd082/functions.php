...
function my_admin_scripts() {
  ...
  wp_enqueue_script('my_script', get_template_directory_uri().'/admin/js/my_script.js', '', '1.0.7', true);
...
function wpdocs_theme_add_editor_styles() {
  ...
  add_editor_style('admin/css/editor-style-04.css');//管理画面用のスタイルシートを変更した場合は、ファイルの名前と番号を変える （キャッシュ対策）
}
...
require get_template_directory() . '/functions/page_design2.php';
require get_template_directory() . '/functions/page_access.php';
...
if ( function_exists('add_theme_support') ) {
  ...
  add_image_size( 'size2', 400, 280, true );
  add_image_size( 'size3', 700, 440, true ); // blog single page
//  add_image_size( 'size4', 900, 400, true ); // service archive page
  add_image_size( 'size5', 1000, 440, true ); // blog single page
...
function message_featured_image_meta_box($content, $post_id, $thumbnail_id) {
  ...
  if ( $post->post_type == 'service') {
    $content .= '<p>' . sprintf(__('Recommend image size. Width:%1$spx, Height:%2$spx.', 'tcd-w'), '1200', '400') . '</p>';
    return $content;
  }
  if ( $post->post_type == 'page') {
    $content .= '<p>' . _e('Recommend image size. Width:500px, Height:500px.<br>This image will be used in search result and OGP tag.', 'tcd-w') . '</p>';
    return $content;
  }
  return $content;
}
...
function tcd_body_classes($classes) {
    ...
    if( is_page() && get_post_meta($post->ID, 'page_hide_logo', true) && get_post_meta($post->ID, 'page_hide_global_menu', true) ) { $classes[] = 'hide_header'; };
    ...
    if( is_page()) {
      ...
      $classes[] = 'page_header_width_' . $page_header_width;
    };
...
<?php endif;  $options = get_option('tcd-w_options'); ?>
     <li class="comment-date"><?php echo get_comment_time(__('Y.m.d', 'tcd-w')); if ($options['time_stamp']) : echo get_comment_time(__(' g:ia', 'tcd-w')); endif; ?></li>
    </ul>
   </div>
...
$faq_category_slug = 'faq_category';
