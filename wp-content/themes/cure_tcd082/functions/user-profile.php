...
?>
  ...
  <h4 class="headline"><?php _e( 'Official position', 'tcd-w' ); ?></h4>
  <div class="input_field">
   <input type="text" name="user_cat" value="<?php echo esc_attr( get_the_author_meta( 'user_cat', $user->ID ) ); ?>" class="regular-text" />
  </div><!-- END input_field -->

  <h4 class="headline"><?php _e( 'Subtitle', 'tcd-w' ); ?></h4>
  <div class="input_field">
   <input type="text" name="user_sub_title" value="<?php echo esc_attr( get_the_author_meta( 'user_sub_title', $user->ID ) ); ?>" class="regular-text" />
  </div><!-- END input_field -->

  <h4 class="headline"><?php _e( 'Message', 'tcd-w' ); ?></h4>
  <div class="input_field">
   <?php wp_editor( wp_kses_post( get_the_author_meta( 'user_message', $user->ID ) , ENT_QUOTES, 'UTF-8'), 'user_message', array( 'wpautop' => true, 'textarea_name' => 'user_message', 'textarea_rows' => 15 ) ); ?>
  </div><!-- END input_field -->

  <h4 class="headline"><?php _e( 'Profile', 'tcd-w' ); ?></h4>
  <div class="theme_option_message2">
   <p><?php _e( 'This option will be used for "Representative member contents" in staff page.', 'tcd-w' ); ?></p>
  </div>
  <div class="input_field">
   <?php wp_editor( wp_kses_post( get_the_author_meta( 'user_prof', $user->ID ) , ENT_QUOTES, 'UTF-8'), 'user_prof', array( 'wpautop' => true, 'textarea_name' => 'user_prof', 'textarea_rows' => 15 ) ); ?>
  </div><!-- END input_field -->
...
?>
  ...
  <div class="input_field">
   <p><?php printf(__('Recommend image size. Width:%1$spx, Height:%2$spx.', 'tcd-w'), '400', '400'); ?></p>
   <?php echo tcd_user_cf_media_form( 'profile_image', $user, 400 ); ?>
  </div><!-- END input_field -->
...
function update_tcdw_profile_fields( $user_id ) {
	...
	if ( user_can( $user_id, 'edit_posts' ) ) {
		$meta_keys = array(
			...
			'user_sub_title' => $_POST['user_sub_title'],
