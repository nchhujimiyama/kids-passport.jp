...
function tcd_google_map( $atts) {
  ...
  if ( $atts['address'] ) {
    ...
    $html .= "  <div id='qt_google_map' class='qt_googlemap_embed'></div>\n";
    $html .= " </div>\n";
    $html .= " <script>\n";
    $html .= " jQuery(function($) {\n";
    $html .= "  $(window).load(function() {\n";
    $html .= "    initMap('qt_google_map', '" . esc_js($atts['address']) . "', " . esc_js($access_saturation) . ", " . esc_js( $use_custom_overlay ) . ", '" . esc_js($marker_img) . "', '" . esc_js($marker_text) . "')\n";
    $html .= "  });\n";
    $html .= " });\n";
    $html .= " </script>\n";
    $html .= "</div>\n";
...
function google_map_scripts() {
  if(is_page() || is_single()){
    global $post, $options;
    if ( ! $options ) $options = get_design_plus_option();
    if( has_shortcode( $post->post_content, 'qt_google_map') ) {
?>
