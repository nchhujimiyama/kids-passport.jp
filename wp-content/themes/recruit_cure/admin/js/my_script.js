jQuery(document).ready(function($){
  ...
  $(window).load(function () {
    $('.cb_user_list .user_list_label').each(function(){
      if( $(this).val() != "" ){
        $(this).closest('.sub_box').find('.theme_option_subbox_headline').text($('option:selected',this).text());
      }
    });
  });
  $('.cb_user_list .user_list_label').change(function(){
    $(this).closest('.sub_box').find('.theme_option_subbox_headline').text($('option:selected',this).text());
  }).trigger('change');
  ...
  // サイドバーのタイプ（CURE）
  $(document).on("change", "#tab-content-basic .footer_bar_icon_type input", function() {
    if($(this).val() == 'tel'){
      $(this).closest('.sub_box_content').find('.side_bar_url').hide();
      $(this).closest('.sub_box_content').find('.side_bar_tel').show();
    } else {
      $(this).closest('.sub_box_content').find('.side_bar_url').show();
      $(this).closest('.sub_box_content').find('.side_bar_tel').hide();
    }
  });


  // ヘッダースライダー　アイテムのタイプ（CURE）
  $(document).on('click', '.index_slider_item_type1', function(event){
    ...
    $(this).closest('.sub_box_content').find('.index_slider_other_setting').show();
  });
  ...
  $(document).on('click', '.index_slider_item_type4', function(event){
    ...
    $(this).closest('.sub_box_content').find('.index_slider_video_area').hide();
    $(this).closest('.sub_box_content').find('.index_slider_youtube_area').hide();
    $(this).closest('.sub_box_content').find('.index_slider_other_setting').hide();
  });
  ...
  $(window).load(function () { //ブロックエディタ対策
    function show_design1_meta_box() {
      ...
      $('#access_meta_box').hide();
      $('#recruit_meta_box').hide();
    ...
    $('select#hidden_page_template').change(function(){ //ブロックエディタ対策
      ...
      } else if ( $(this).val() == 'page-design2.php' ) {
        show_design2_meta_box();
      } else if ( $(this).val() == 'page-access.php' ) {
				show_access_meta_box();
			} else if ( $(this).val() == 'page-hybrid.php' ) {
				show_hybrid_meta_box()
      } else {
        normal_template();
      }
    }).trigger('change');
