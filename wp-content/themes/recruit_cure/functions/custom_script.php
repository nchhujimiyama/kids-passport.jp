...
function custom_script_meta_box() {
  ...
  add_meta_box(
    ...
    'カスタムスクリプト（この固定ページのみに適用されます）',//label
...
function show_custom_script_meta_box() {
  ...
  $custom_css = get_post_meta($post->ID, 'custom_script', true);
...
<div class="tcd_custom_fields">
 ...
 <div class="tcd_cf_content">
  <div class="theme_option_message2" style="margin-top:10px;">
   <p>以下のScriptは&lt;head&gt;タグ内に出力されます。<br>&lt;script&gt;タグを入力する必要はございません。</p>
  </div>
...
function add_custom_script_footer(){
  ...
  $script = get_post_meta(get_the_ID(), 'custom_script', true);
  if( $script ) {
    echo $script;
  }
};
