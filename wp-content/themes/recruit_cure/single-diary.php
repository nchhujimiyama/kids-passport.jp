...
<div id="main_contents" class="clearfix">
  ...
  <div id="main_col">
    ...
    <?php if ( have_posts() ) : while ( have_posts() ) : the_post(); ?>
      ...
      <article id="article">
        ...
        <?php if($page == '1') { // ***** only show on first page *****
          ...
          if( $options['single_diary_layout'] == 'type3'){
          ...
          ?>
          <div id="post_title_area"<?php if( (!has_post_thumbnail()) || (!$options['single_diary_show_image']) ) { echo ' class="no_image"'; }; ?>>
            <div class="image_area">
              <?php if($options['single_diary_show_image'] && has_post_thumbnail()) { ?>
              ...
              $blog_category = wp_get_post_terms( $post->ID, 'diary_category' , array( 'orderby' => 'term_order' ));
              ...
              if( $blog_cat_id && $blog_cat_name ) { ?>
                <p class="category cat_id_<?php echo esc_attr($blog_cat_id); ?>"><a href="<?php echo esc_url(get_term_link($blog_cat_id,'diary_category')); ?>"><?php echo esc_html($blog_cat_name); ?></a></p>
              <?php } ?>
            </div>

            <h1 class="title rich_font_<?php echo esc_attr($options['single_diary_title_font_type']); ?> entry-title"><?php the_title(); ?></h1>
            <?php if ( $options['single_diary_show_date'] ){ ?>
          ...
          <?php
          // sns button top ------------------------------------------------------------------------------------------------------------------------
          if($options['single_diary_show_sns_top']) { ?>
          ...
          if($options['single_diary_show_copy_top']) { ?>
        ...
        // sns button ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        if($options['single_diary_show_sns_btm']) { ?>
        ...
        // copy title&url button ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        if($options['single_diary_show_copy_btm']) { ?>
        ...
        // meta ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        if ($options['single_diary_show_meta_box']) { ?>
          <ul id="post_meta_bottom" class="clearfix">
            <?php if ($options['single_diary_show_meta_author']) : ?><li class="post_author"><?php _e("Author","tcd-w"); ?>: <?php if (function_exists('coauthors_posts_links')) { coauthors_posts_links(', ',', ','','',true); } else { the_author_posts_link(); }; ?></li><?php endif; ?>
            <?php if ($options['single_diary_show_meta_category']){ ?><li class="post_category"><?php the_taxonomies(', '); ?></li><?php }; ?>
            <?php if ($options['single_diary_show_meta_tag']): ?><?php the_tags('<li class="post_tag">',', ','</li>'); ?><?php endif; ?>
            <?php if ($options['single_diary_show_meta_comment']) : if (comments_open()){ ?><li class="post_comment"><?php _e("Comment","tcd-w"); ?>: <a href="#comments"><?php comments_number( '0','1','%' ); ?></a></li><?php }; endif; ?>
          </ul>
        <?php };

        // page nav ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        if ($options['single_diary_show_nav']) : ?>
    ...
    if ($options['show_related_post']){
      $categories = get_the_terms($post->ID, 'diary_category');
      if ($categories) {
    ...
    // comment ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    if ($options['single_diary_show_comment'] || $options['single_diary_show_trackback']) { comments_template('', true); }; ?>
